...
def make_prediction(nn_path, im_path):
    sample = Image.open(im_path)
    ...
    # Реализовать те же операции с W и b, чтобы получить Х, либо перетащить из модели
    with tf.Session() as sess:
        saver = tf.train.import_meta_graph(saved_nn + '.meta')
        saver.restore(sess, saved_nn)
...
def count_accuracy(nn_path , data_path):
    ...
    for file in files:
        image, prediction = make_prediction(nn_path, data_path + file)
        ans, res_list = analyze_prediction(prediction)
        if str(ans) in file:
            counter_accuracy += 1
        for res in res_list:
            if res in file:
...
saved_nn = 'C:\\Users\\User\\Desktop\\Thesis\\trained_nn\\bigbatch\\bigbatch.ckpt-4126'
...
count_accuracy(saved_nn, data_path)
